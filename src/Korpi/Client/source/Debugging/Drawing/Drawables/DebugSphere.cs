...
public class DebugSphere : DebugDrawable
{
    ...
    static DebugSphere()
    {
        ...
        {
            double lat = System.Math.PI * i / precision;
            for (int j = 0; j <= precision; j++)
            {
                double lon = 2 * System.Math.PI * j / precision;
                float x = (float)(System.Math.Cos(lon) * System.Math.Sin(lat));
                float y = (float)System.Math.Cos(lat);
                float z = (float)(System.Math.Sin(lon) * System.Math.Sin(lat));
