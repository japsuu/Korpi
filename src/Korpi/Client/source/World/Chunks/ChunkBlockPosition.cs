...
public readonly struct ChunkBlockPosition
{
    ...
    public ChunkBlockPosition(int x, int y, int z)
    {
        ...
        Index = x + (y << Constants.CHUNK_SIDE_BITS) + (z << Constants.CHUNK_SIDE_BITS * 2);
        X = x;
        Y = y;
        Z = z;
    }
    ...
    public ChunkBlockPosition(Vector3i position)
    {
        ...
        Index = position.X + (position.Y << Constants.CHUNK_SIDE_BITS) + (position.Z << Constants.CHUNK_SIDE_BITS * 2);
        X = position.X;
        Y = position.Y;
        Z = position.Z;
    }
