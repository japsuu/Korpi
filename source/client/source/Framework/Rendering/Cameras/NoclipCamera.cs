...
public class NoclipCamera : Camera
{
    ...
    private void UpdatePosition()
    {
        ...
        {
            SetPosition(Position + Forward * _cameraFlySpeed * (float)Time.DeltaTime); // Forward
        }

        if (Input.KeyboardState.IsKeyDown(Keys.S))
        {
            SetPosition(Position - Forward * _cameraFlySpeed * (float)Time.DeltaTime); // Backward
        }

        if (Input.KeyboardState.IsKeyDown(Keys.A))
        {
            SetPosition(Position - Right * _cameraFlySpeed * (float)Time.DeltaTime); // Left
        }

        if (Input.KeyboardState.IsKeyDown(Keys.D))
        {
            SetPosition(Position + Right * _cameraFlySpeed * (float)Time.DeltaTime); // Right
        }

        if (Input.KeyboardState.IsKeyDown(Keys.Space))
        {
            SetPosition(Position + Up * _cameraFlySpeed * (float)Time.DeltaTime); // Up
        }

        if (Input.KeyboardState.IsKeyDown(Keys.LeftShift))
        {
            SetPosition(Position - Up * _cameraFlySpeed * (float)Time.DeltaTime); // Down
        }
    }
